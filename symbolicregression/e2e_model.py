...
# Custom pickle module to handle cross-platform path issues (PosixPath on Windows)
if sys.platform == "win32":
    import pathlib

    # Make PosixPath available on Windows for unpickling
    if not hasattr(pathlib, "PosixPath"):
        pathlib.PosixPath = Path


# Custom unpickler class for cross-platform compatibility
class CrossPlatformUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        # Handle pathlib.Path cross-platform issue
        if module == "pathlib" and name == "PosixPath":
            return Path  # Use current system's Path
        return super().find_class(module, name)


# Create a custom pickle module
class CrossPlatformPickleModule:
    Unpickler = CrossPlatformUnpickler


# Copy other required attributes from pickle module
for attr in dir(pickle):
    if not attr.startswith("_") and not hasattr(CrossPlatformPickleModule, attr):
        try:
            setattr(CrossPlatformPickleModule, attr, getattr(pickle, attr))
        except:
            pass
...
class Transformer(nn.Module):
    def __init__(self, params, env, samples):
        ...
        # Support CPU: map model to target device
        map_location = getattr(params, "device", "cpu")
        # Handle cross-platform path issues (PosixPath on Windows)
        # Try loading with custom pickle module if available
        try:
            # Try with custom pickle module (for PyTorch versions that support it)
            self.model = torch.load(
                model_path,
                map_location=map_location,
                pickle_module=CrossPlatformPickleModule,
            )
        except TypeError:
            # Fallback: use standard load (pathlib should be patched at module level)
            self.model = torch.load(model_path, map_location=map_location)
        self.first_dropout = nn.Dropout(0.1)
        self.params = params
        self.env = env
        self.embedder, self.encoder, self.decoder = (
            self.model.embedder,
            self.model.encoder,
            self.model.decoder,
        )
        ...
        for seq_id in range(len(x_to_fit)):
            ...
            for seq_l in range(len(x_to_fit[seq_id])):
                if np.isscalar(y_to_fit[seq_id][seq_l]):
                    x1[seq_id].append(
                        [x_to_fit[seq_id][seq_l], np.array([y_to_fit[seq_id][seq_l]])]
                    )
                else:
                    x1[seq_id].append(
        ...
        self.encoded = self.encoder(
            "fwd", x=self.x1, lengths=self.src_len, causal=False
        ).transpose(0, 1)
    ...
    def generate_beams(
        ...
        decoded, tgt_len, generated_hyps, top_k_hash = (
            self.decoder.generate_beam_from_state(
                self.encoded,
                self.src_len,
                input_ids,
                num_beams,
...
def pred_for_sample(
    model, env, x_to_fit, y_to_fit, refine=True, beam_type="search", beam_size=1
):
    mw = ModelWrapper(
        env=env,
        embedder=model.embedder,
    ...
    if refine:
        ...
        tree = dstr.retrieve_tree(refinement_type="NoRef", with_infos=True)[
...
def refine_for_sample(params, model, env, preds, x_to_fit, y_to_fit):
    ...
    # Handle invalid tokens gracefully (can occur during RL exploration)
    try:
        generations_tree = list(
            filter(
                lambda x: x is not None,
        ...
        # Invalid tokens in sequence - return empty list (will result in None prediction and penalty reward)
        # This is expected during MCTS exploration when model generates out-of-vocabulary tokens
        generations_tree = []
    ...
    if generations_tree == []:
        ...
        for i in range(n_datasets):
            dstr.top_k_features[i] = get_top_k_features(
                X[i], Y[i], k=dstr.model.env.params.max_input_dimension
            )
